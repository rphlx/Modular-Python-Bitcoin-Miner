...
###################################################
# rph usb hotplug controller module               #
...
class rphUSBHotplugWorker(BaseWorker):
  
  version = "rph.usb hotplug manager v0.0.1"
  default_name = "rph usb hotplug manager"
  can_autodetect = True
        ...
        for bus in usb.busses():
          for dev in bus.devices:
            if dev.idVendor == 0xf1f0:
              try:
                handle = dev.open()
                serial = hexlify(handle.controlMsg(0xc0, 0x96, 16, 0, 0, 100))
                try:
  ...
  def __init__(self, core, state = None):
    ...
    super(rphUSBHotplugWorker, self).__init__(core, state)
  ...
  def apply_settings(self):
    ...
    super(rphUSBHotplugWorker, self).apply_settings()
  ...
  def _reset(self):
    # Let our superclass handle everything that isn't specific to this worker module
    super(rphUSBHotplugWorker, self)._reset()
  ...
  def _start(self):
    # Let our superclass handle everything that isn't specific to this worker module
    super(rphUSBHotplugWorker, self)._start()
  ...
  def _stop(self):
    # Let our superclass handle everything that isn't specific to this worker module
    super(rphUSBHotplugWorker, self)._stop()
    ...
    while not self.shutdown:
      ...
      try:
        ...
        for serial, available in boards.items():
          ...
          if available:
            child = rphUSBWorker(self.core)
            child.settings.name = "rph USB " + serial
